# sprite-renderer Specification

## Purpose

定义 `SpriteSheetRenderer` —— 基于 HTML Canvas 2D 的逐帧渲染器实现 `IRenderer` 接口，并通过 `requestAnimationFrame` 驱动渲染循环，配合 `SpriteSheetAnimationEngine` 输出帧画面与表情覆盖层。
