...
The system SHALL size the main window to exactly match the Live2D model display dimensions (100×100 pixels by default). No additional padding or margins SHALL exist around the model area.

#### Scenario: Window size matches model size
- **WHEN** the application starts with default model dimensions
- **THEN** the window width SHALL be 100 pixels
- **THEN** the window height SHALL be 100 pixels
